    ...
    try:
        if (
            ...
            result_id = db.execute(
                "SELECT id FROM sprav_result WHERE name = ?", result
            )[0]["id"]
            student_id = db.execute(
                "SELECT id FROM students WHERE student_number = ?", stud_id
            )[0]["id"]
...
def get_history():
    return db.execute(
        "SELECT history.id, history.action, history.details, history.dt FROM history;"
    )
...
def update_history(action, details, dt):
    try:
        db.execute(
            "INSERT INTO history (action, details, dt) VALUES (?, ?, ?)",
            action,
            details,
            dt,
        )
